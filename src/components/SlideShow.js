...
<Grid container>
<Grid item xs={12}>
<Slider className="slider-wrapper"  infinite="false" autoplay="2000 (ms)" >
{content.map((item, index) => (
  <Grid item
    key={index}
    className="slider-content"
    style={{ background: `url('${item.image}') no-repeat center center`, backgroundSize: 'cover', position: 'fixed' }}
  >
    <Grid item className="inner">
      <h1>{item.title}</h1>
      <p>{item.description}</p>
      <StyledButton component={Link} to={item.link}>Click Here</StyledButton>
          </Grid>
  </Grid>
))}
</Slider>
